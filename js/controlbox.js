define(['vendor/yargs-parser', 'd3', 'demos'], function (_yargs, d3, demos) {
	'use strict';
	...
	ControlBox.prototype = {
		...
		render: function (container) {
			...
			selector = cBoxContainer.append('select').classed('scenario-chooser', true);

			demos.forEach(function (demo) {
				var opt = selector.append('option').text(demo.title).attr('value', demo.key);
			...
			log = cBoxContainer.append('div').classed('log', true);
			...
			input.on('keyup', function () {
				...
				switch (e.keyCode) {
					...
					case 40:
						...
						break;
					default:
						var lastCommandElement = document.getElementById('last-command');
						if (lastCommandElement) {
							lastCommandElement.textContent =
								document.querySelectorAll('.control-box .input')[0].value;
						}
				}
			});
		...
		command: function (entry) {
			...
			var lastCommandElement = document.getElementById('last-command');
			if (lastCommandElement) {
				lastCommandElement.textContent = entry;
			}
			...
			if (entry.toLowerCase() === 'undo') {
				...
				this.persist();
				this.terminalOutput.append('div').classed('command-entry', true).html(entry);
		...
		commit: function (args, opts, cmdStr) {
			...
			this.transact(
				...
				function (before, after) {
					...
					this.getRepoView().addReflogEntry('HEAD', after.commit.id, reflogMsg);
					if (before.branch) {
						this.getRepoView().addReflogEntry(before.branch, after.commit.id, reflogMsg);
					}
				}
			);
		},
		...
		log: function (args) {
			...
			var logs = this.getRepoView()
				.getLogEntries(args[0] || 'head')
				.map(function (l) {
			...
			this.transact(
				...
				function (before, after) {
					this.getRepoView().addReflogEntry(
						'HEAD',
						after.commit.id,
						'checkout: moving from ' + before.ref + ' to ' + name
					);
				}
			);
		},
